...
export class ProductStore {
  ...
  get categories() { return [...new Set(this.products.map((p) => p.category))].sort(); }
  get isListCacheValid() {
    return this.products.length > 0 && this.listFetchedAt !== null && Date.now() - this.listFetchedAt < this.LIST_CACHE_TTL;
  }
  isItemCacheValid(id: number) {
  ...
  async fetchProducts(force = false) {
    ...
    try {
      ...
      runInAction(() => {
        this.products = res.data; this.listFetchedAt = Date.now();
        res.data.forEach((p) => this.productCache.set(p.id, { product: p, fetchedAt: Date.now() }));
      });
    } catch { runInAction(() => { this.error = 'Не удалось загрузить товары'; }); }
    finally { runInAction(() => { this.isLoading = false; }); }
  }

  async fetchProduct(id: number, force = false) {
    if (!force && this.isItemCacheValid(id)) {
      runInAction(() => { this.selectedProduct = this.productCache.get(id)!.product; });
      return;
    }
    this.isLoadingOne = true; this.error = null;
    try {
    ...
    } catch { runInAction(() => { this.error = 'Товар не найден'; }); }
    finally { runInAction(() => { this.isLoadingOne = false; }); }
  }
  ...
  clearSelected() { this.selectedProduct = null; }
}
