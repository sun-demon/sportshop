...
import { useCreateProductMutation, useGetProductQuery, useUpdateProductMutation } from '../api/sportshopApi';
...
export default function ProductFormPage() {
  ...
  useEffect(() => {
    if (existing) {
      ...
      setPrice(String(existing.price)); setStock(String(existing.stock));
      setCategory(existing.category); setImageUrl(existing.imageUrl ?? '');
    }
  }, [existing]);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      if (isEdit) { await updateProduct({ id: Number(id), data }).unwrap(); navigate(`/products/${id}`); }
      else { const p = await createProduct(data).unwrap(); navigate(`/products/${p.id}`); }
    } catch (err: unknown) {
      ...
      <div className="form-page">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-actions">
            ...
            <button type="submit" className="btn btn-primary" disabled={creating || updating}>{creating || updating ? 'Сохранение…' : 'Сохранить'}</button>
          </div>
        </form>
      </div>
    </div>
