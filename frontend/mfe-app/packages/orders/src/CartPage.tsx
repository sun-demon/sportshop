    ...
    <div className="container">
      ...
      <div className="cart-layout">
        <div className="cart-items">
          {items.map(({ product, quantity }) => (
            <div key={product.id} className="cart-item">
              ...
              <button type="button" className="btn btn-danger btn-sm" onClick={() => dispatch(removeFromCart(product.id))}>
                ✕
              </button>
            </div>
          ))}
        ...
        <div className="cart-summary">
          ...
          <button type="button" className="btn btn-primary btn-full" onClick={handleCheckout} disabled={isLoading}>
            {isLoading ? 'Оформление…' : 'Оформить заказ'}
          </button>
          <button type="button" className="btn btn-outline btn-full" onClick={() => dispatch(clearCart())}>
            Очистить корзину
          </button>
        </div>
      </div>
